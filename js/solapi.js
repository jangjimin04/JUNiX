...
return evaluateBooth;
  ...
  function btn_sendMessage(){
    ...
    let btn_url1 = "ksyeong0316.dothome.co.kr/0513/index_subpage.html" // 상담예약 및 조회 및 변경
    let btn_url2 = "https://juniklee.github.io/testing-11/" // 샘플 페이지 바로가기
    let btn_url3 = "ksyeong0316.dothome.co.kr/0513/index_dummy.html" // 홈페이지 바로가기 
    ...
    let input4 = document.querySelector("#Type4");
    
    if(name == ""){
        alert("성함을 입력해 주시길 바랍니다");
        return;}
    if(phonenumber == ""){
        alert("전화번호를 입력해 주시길 바랍니다");
        return;}
    if(companyname == ""){
        alert("회사명을  입력해 주시길 바랍니다");
        return;}
    if(email == ""){
        alert("이메일을  입력해 주시길 바랍니다");
        return;}
    if(!input4.checked){
        alert("개인 정보제공을 동의해 주시길 바랍니다");
        return;}
